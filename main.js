// //body-parser : POST로 요청했을 때의 요청 패러미터 확인 방법 제공
...
//     console.log('첫 번째 파라미터에서 요청을 처리함');
...
router.route('/process/login').post(function (req, res) {
    ...
    var paramUserId = req.body.userId || req.query.userId;
    var paramUserPwd = req.body.userPwd || req.query.userPwd;
    if (req.session.user) {   //이미 로그인 된 상태
...
router.route('/process/product').get(function (req, res) {
    ...
    if (req.session.user) {
        ...
        console.log('로그인 상태가 아니라서 로그인 창으로 넘어옴');
        res.redirect('/public/login2.html');
    }
})
...
router.route('/process/upload').post(upload.array('uploadedFile', 1), function (req, res) {
    ...
    try {
        var files = req.files; console.dir('#=== 업로드 된 첫번째 파일 정보 ===#');
        ...
        if (Array.isArray(files)) {
            ...
            console.log("파일 갯수 : 1");
            originalname = files[index].originalname; filename = files[index].name;
            mimetype = files[index].mimetype; size = files[index].size;
        }
